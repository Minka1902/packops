...
function MedicalContent({ event }: { event: MedicalCalendarEvent }) {
  ...
  const specificField = (() => {
    if (record.category === 'vaccination')  return (record as import('@/shared/types').Vaccination).vaccineName;
    if (record.category === 'medication')   return (record as import('@/shared/types').Medication).medicationName;
    if (record.category === 'flea_tick')    return (record as import('@/shared/types').FleaTick).productName;
    if (record.category === 'deworming')    return (record as import('@/shared/types').Deworming).productName;
    if (record.category === 'allergy')      return (record as import('@/shared/types').Allergy).allergen;
    if (record.category === 'diagnosis')    return (record as import('@/shared/types').Diagnosis).condition;
    if (record.category === 'surgery')      return (record as import('@/shared/types').Surgery).procedure;
    return undefined;
  })();
  ...
  return (
    <>
      ...
      <div className="px-4 pb-4">
        ...
        {record.category === 'medication' && (record as import('@/shared/types').Medication).dosage && (
          <Row icon={Droplets} label="Dosage" value={(record as import('@/shared/types').Medication).dosage!} />
        )}
        {record.category === 'medication' && (record as import('@/shared/types').Medication).frequency && (
          <Row icon={Clock} label="Frequency" value={(record as import('@/shared/types').Medication).frequency!} />
        )}
        {record.category === 'allergy' && (record as import('@/shared/types').Allergy).severity && (
          <Row icon={AlertCircle} label="Severity" value={(record as import('@/shared/types').Allergy).severity!} />
        )}
